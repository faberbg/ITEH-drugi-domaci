...
<style>
  ...
  height: 100%;
  position: relative;
  background: url("{{url('/images/grad.jpg')}}");
...
@section('content')
    ...
    <div class="container">
        <div class="title">
          <h1><Strong>Gradovi</strong></h1>
        </div>

        <ul class="list">
        @foreach($grad as $grad)
          <li>
            <div ><h2 style="text-align:center">{{$grad->imegrada}}</h2></div>
           
            <div class="btn" name="select" type="select" data-submit="...Sending"><a href="{{$grad->id}}">Poslovi</a></div>
          </li>
          @endforeach
        </ul>
      </div>
